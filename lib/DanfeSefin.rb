...
module RubyDanfe
    ...
    class DanfeGenerator
      ...
      def render_emitente
        ...
        @pdf.ibarcode 1.50, 8.00, 10.4010, 4.44, @xml['//infProt/chNFe']
        @pdf.ibox 0.85, 11.02, 9.79, 4.74, "CHAVE DE ACESSO", @xml['//infProt/chNFe'].gsub(/\D/, '').gsub(/(\d)(?=(\d\d\d\d)+(?!\d))/, "\\1 "), {:style => :bold, :align => :center}
        ...
        if @xml['emit/CNPJ'].blank? 
          @pdf.ibox 0.85, 6.84, 13.97, 7.31, "CPF", @xml['emit/CPF']
        else
      ...
      def render_dados_adicionais_box
        @pdf.ititle 0.42, 10.00, 0.25, 25.91, "DADOS ADICIONAIS"
         @pdf.ibox 3.07, 12.93, 0.25, 26.33, "INFORMAÇÕES COMPLEMENTARES",
            @xml['infAdic/infCpl'], {:size => 8, :valign => :top}
  
        @pdf.ibox 3.07, 7.62, 13.17, 26.33, "RESERVADO AO FISCO",
            @xml['infAdic/infAdFisco'], {:size => 8, :valign => :top}
      end
